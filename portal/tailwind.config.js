  ...
  theme: {
    extend: {
      ...
      animation: {
         ...
         * only there, by the `Waiting` primitive every wait in the portal renders through — and
         * `MotionConfig reducedMotion="user"` at the root in `src/main.tsx`, which is what
         * reaches the `motion` library's JS-driven animation, are the portal's only three
         * mechanisms; there is no fourth, and `src/__tests__/reducedMotion.test.ts` pins that
         * fact so it cannot again be claimed universal while dozens of other spinners quietly
         * ignore the preference. The hook's branch was previously copied into three components
         * and every copy was wrong the same way — it stopped the spin and kept the spinner,
         * which is a picture of a hang.
         */
