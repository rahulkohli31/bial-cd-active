 ...
 * leaving takes it away. One fact a screen cannot infer from that is stated here: the container
 * has a ceiling nothing postpones.
 */
...
describe('the closing-soon note', () => {
  it('names the time the app closes when the ceiling is near', () => {
    render(<WorkspaceLifecycleNotes drainingAt={inMinutes(10)} />)
  ...
  it('says NOTHING about a ceiling that is hours away', () => {
    ...
    render(<WorkspaceLifecycleNotes drainingAt={inMinutes(115)} />)
  ...
  it('★ says nothing at all when no ceiling applies', () => {
    ...
    render(<WorkspaceLifecycleNotes drainingAt={null} />)

    expect(screen.queryByTestId('workspace-lifecycle-notes')).toBeNull()
  })

  it('says nothing when the instant is not readable as one', () => {
    render(<WorkspaceLifecycleNotes drainingAt="not-a-date" />)

    expect(screen.queryByTestId('closing-soon-note')).toBeNull()
  })

  it('announces politely — it interrupts nothing', () => {
    render(<WorkspaceLifecycleNotes drainingAt={inMinutes(10)} />)
  ...
  it('★ says nothing once the ceiling has passed', () => {
    ...
    render(<WorkspaceLifecycleNotes drainingAt={inMinutes(-5)} />)
    expect(screen.queryByTestId('closing-soon-note')).toBeNull()
