 ...
 * `/projects`: they are components mounted on a *different* address (`/projects/:id`,
 * `/chat/:id`) that have to name a destination without ever having read the list's own query
 * string themselves. A bare `/projects` from any of them reopens the list at page one with the
 * search cleared.
 *
 * `AppShell` frames every route, so it is the one place positioned to WRITE this on every render
 * where the address bar reads `/projects`. `WorkspaceShell`'s back control, the navigation's list
 * entry and its brand link all READ it when constructing a destination — at CLICK time, so a
 ...
 * that only exercises a read still passes, and the list silently reopens at page one. That is why
 * the write has a scenario of its own in `AppShell.test.tsx`.
 *
 ...
 *
 * ONE EXPORT RATHER THAN A CONCATENATION AT EACH CALLER. Three controls point at this list — the
 * workspace back control, the navigation's list entry and its brand mark — and a route name plus
 * a concatenation rule spelled three times fails silently when only one is edited.
 */
