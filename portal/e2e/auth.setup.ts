 ...
 * and `backend/mint_dev_session.py` issues them for an existing user row
 ...
 *
 *     cd backend && uv run python mint_dev_session.py \
  ...
  if (requireReal) {
    throw new Error(
      'E2E_REQUIRE_REAL_SESSION=1 but no usable E2E_STORAGE_STATE. Mint one first:\n' +
        '  cd backend && uv run python mint_dev_session.py --email <addr> --out <path>\n' +
