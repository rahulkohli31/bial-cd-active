...
async def reconcile_user(
    ...
    *,
    ...
    """Reconcile the user's OWN stale state before a fresh start. Reap ONLY when a
    registry entry exists AND this process holds NO live in-process session for the user
    (load-bearing: `run_build` outlives the SSE disconnect, so a live multi-minute build
    whose tab closed >90 s would otherwise be reaped mid-flight) AND (the lock is gone OR
    the heartbeat has lapsed). Returns True if it reaped."""
    if has_live_session:
    ...
    *,
    ...
    async for raw_key in redis.scan_iter(match=_REGISTRY_SCAN_MATCH):
        ...
        if user_uuid is None or user_uuid in live:
            continue
        if await reconcile_user(redis, user_uuid, sandbox_client, has_live_session=False):
            reaped += 1
    return reaped
