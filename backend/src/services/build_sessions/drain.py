...
def the_ceiling_hours() -> int | None:
    """How many hours a container may live, or `None` when no sandbox is configured.
    ...
    if settings.sandbox is None:
        return None
    return settings.sandbox.drain_after_hours


def draining_at(identity: SandboxIdentity, *, after_hours: int | None) -> dt.datetime | None:
    """When this container will be drained, or `None` if it will not be.

    `None` when no sandbox is configured, or when the container carries no trustworthy age. A
    caller renders this to the builder, so it answers "when", never "whether" — a boolean would
    leave the UI inventing the sentence.
    ...
    if after_hours is None or identity.created_at is None:
        return None
...
def is_drained(
    ...
    *,
    ...
    if turn_in_flight:
        return False
    mark = draining_at(identity, after_hours=after_hours)
