...
def test_chain_ends_at_a_single_linear_head() -> None:
    ...
    # 0024_messages_native_reset through 0025_deployments and 0026_deployment_classification
    # (the declaration that authorised each deploy) and 0027_worker_passes, to
    # 0028_drop_login_required (issue #92, R22 — dropping the dead app_registry.login_required
    # column). Pinning the exact head — rather than just the COUNT, which
    # `test_alembic_single_head.py` already guards — is what makes a rebase that silently
    # re-parents a revision fail here instead of at deploy. Updating this line is the
    # deliberate acknowledgement that a new migration landed; if you are here because it
    # failed, check that your revision's `down_revision` really is the head you expected to
    # build on.
    config = Config(str(_BACKEND_ROOT / "alembic.ini"))
    heads = ScriptDirectory.from_config(config).get_heads()
    assert heads == ["0028_drop_login_required"]
