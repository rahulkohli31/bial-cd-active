...
OWN records API with the app's `X-App-Key` header (the U4 chain). This journey drives that
surface end-to-end the way a running generated app does (the old per-app file surface was
retired with the open-sandbox pivot, so the data plane is records-only):
  ...
  3. cross-app ISOLATION: app B's key cannot reach app A's records — every cross-boundary
     read fails CLOSED with 404 (ADR-0004: a dropped app_id predicate is a cross-app leak).
...
async def test_generated_app_data_plane_end_to_end(client, app, db_session) -> None:
    # The data plane is records-only since the old per-app file surface was retired (U10).
    ...
    # ================================================================
    # PHASE 2 — cross-app ISOLATION (fail-closed 404, no cross-app leak)
    # ================================================================

    app_b, key_b = await _approved_app(db_session)

    # 2a. App B's key on App A's records URL → 404 (the appkey-chain URL/key cross-check).
    ...
    # 2b. App B addressing A's record id under B's OWN app URL → 404 (app-scoped read).
    cross_scoped = await client.get(f"/v1/apps/{app_b.id}/records/{r1_id}", headers=key_b)
    assert cross_scoped.status_code == 404

    # 2c. App B never sees A's rows: its own list is empty.
