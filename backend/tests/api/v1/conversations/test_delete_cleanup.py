...
async def test_delete_sweeps_a_code_lane_file_too(client, db_session, fake_storage) -> None:
    """★ THE SECOND MARKER, WHICH THE CASCADE HAS TO KNOW ABOUT TOO.

    A model-lane file leaves `bial-attachment-ref` because its bytes were externalized out of a
    `BinaryContent`. A code-lane file never becomes one — it is written into the container and
    named in prose — so it leaves `bial-attachment-file-ref` instead, and a cascade that walked
    only the first marker left the row and the stored spreadsheet behind on every deleted chat,
    for as long as the platform runs.

    Mutation receipt: drop the file-ref kind from the cascade's walk and the row and the object
    below both survive the delete.
    """
    from src.services.messages.store import ATTACHMENT_FILE_REF_KIND

    headers, user = await _auth(db_session)
    conv = await ConversationFactory.create(db_session, user.id)
    key = f"att/{user.id}/att_book"
    db_session.add(
        Attachment(
            user_id=user.id,
            attachment_id="att_book",
    ...
    fake_storage.objects[key] = b"PK\x03\x04"
    # The payload shape `append_batch` writes for a code-lane file: the marker rides in the
    # user-prompt content list, where no `BinaryContent` ever was.
    await MessageFactory.create(
        db_session,
        user.id,
        conv.id,
                ...
                "parts": [
                    {
                        ...
                        "content": [
                            ...
                            {"kind": ATTACHMENT_FILE_REF_KIND, "attachment_id": "att_book"},
                        ],
                    }
                ],
            }
    ...
    assert resp.status_code == 200
    assert (
        await db_session.scalar(select(Attachment).where(Attachment.attachment_id == "att_book"))
        is None
    )
    assert fake_storage.objects == {}
